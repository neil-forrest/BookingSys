...
function CreatePreviousMonth($roomtype,$number){
...
global $pdo;
 ...
 $sql = ("SELECT * FROM bookings WHERE date=:date and roomtype_id=:roomtype");
...
public function CreateMonth($roomtype,$number){
...
public function CreateNextMonth($roomtype,$number){
...
echo '<div class="row">';
   ...
   echo '<div class="col-md-4"><form method="post" action="'. $_SERVER['PHP_SELF'].'"> 
        ...
        </form>  </div><div class="col-md-4"><h2>Select Room Type</h2>';
...
<input type="submit" value="'.$roomtype['roomtype'].'"></form>';
 }
echo '</div></div>';
...
$stmt = $pdo->prepare($sql);#
$stmt->bindParam(':id', $roomtype, PDO::PARAM_INT);
$stmt->execute();
$room = $stmt->fetchObject();
echo '<h2>' . $room->roomtype . '</h2>';
$number=$room->number_rooms;
$calendar->CreateDatepicker(10);
echo'<form action="details.php" method="POST">
<input type="hidden" name="roomtype" value="' .$roomtype.'">
<div id="CreatePreviousMonth" class="col-md-4">';
$calendar->CreatePreviousMonth($roomtype,$number);
echo'</div><div id="CreateMonth" class="col-md-4">';
$calendar->CreateMonth($roomtype,$number);
echo'</div><div id="CreateNextMonth" class="col-md-4">';
$calendar->CreateNextMonth($roomtype,$number);
